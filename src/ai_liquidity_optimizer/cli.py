...
def _summarize_once_result_ev(result: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"stage={result.get('market_state_stage')} "
        f"regime={result.get('market_regime')} "
        f"reg_conf={_fmt_num(result.get('regime_confidence'), 3)} "
        f"agree={_fmt_num(result.get('horizon_agreement_score'), 3)} "
        f"size={_fmt_num(result.get('size_multiplier'), 2)} "
        f"family={result.get('candidate_family')} "
        f"re15={_fmt_num(result.get('reentry_prob_15m'), 3)} "
        f"re1h={_fmt_num(result.get('reentry_prob_1h'), 3)} "
        ...
        f"untracked_pool_positions={gate.get('untracked_pool_positions_count')} "
        ...
        f"regime_cal={realism_snapshot.get('regime_label')}:{realism_snapshot.get('regime_sample_count')} "
        f"top_bins={top_bins}"
    )
